...
GROUP BY DEPT_CODE
ORDER BY 1; -- 오류발생 (그룹함수를 가지고 조건 제시시 WHERE 절에서는 안됨!)
...
-- 직급별 총 급여합(단, 직급별 급여합이 1000만원 이싱인 직급만을 조회)
...
-- 부서별 보너스를 받는 사원이 없는 부서만을 조회 (부서코드, 보너스를 몇명이 받는지)
...
/*
    ...
    - UNION ALL : 합집합 + 교집합 (중복되는 부분이 두 번 표현 될 수 있음!)
...
WHERE SALARY > 3000000
ORDER BY EMP_NAME;
-- ORDER BY절을 붙이고자 한다면 마지막에 기술해야됨!
