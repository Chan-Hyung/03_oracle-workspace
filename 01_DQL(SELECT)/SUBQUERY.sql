...
/*
    * 서브쿼리의 구분
      ...
      - 다중행 서브쿼리 : 서브쿼리의 조회 결과값이 여러행일 때(여러행 한열) => 동명이인 노옹철 2명일 때
      - 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 한 행지만 컬럼이 여러개일 때 (한 행 여러열)
      - 다중행 다중열 서브쿼리 : 서브쿼리 조회 결과값이 여러행 여러컬럼일 때 (여러행 어러열)
...
WHERE JOB_CODE IN (SELECT JOB_CODE
                    FROM EMPLOYEE
                    WHERE EMP_NAME IN ('유재식', '윤은해')); -- = 으로 쓰면 오류남!! 여러행으로 조회됐기 때문!!

-- 사원 => 대리 => 과장 => 차장 => 부장 ...
-- 2) 대리 직급임에도 불구하고 과장 직급 급여들 중 최소급여 보다 더 많이 받는 직원 조회 (사번, 이름, 직급, 급여)
...
-- 2_2) 직급이 대리이면서 급여값이 위의 목록들 값중에 하나라도 큰 사원
...
SELECT EMP_ID, EMP_NAME --, MANAGER_ID : 인라인 뷰에 없는 컬럼 제시하면 오류발생
FROM (SELECT EMP_ID, EMP_NAME, SALARY + SALARY * NVL(BONUS, 0)*12 AS "연봉", DEPT_CODE
        FROM EMPLOYEE)
WHERE 연봉 >= 30000000;

-- >> 인라인뷰를 주로 사용하는 예 => TOP-N 분석 (상위 몇개만 보여주고 싶을 때 => BEST 상품, BEST 게시글...)

-- 전 직원 중 급여가 가장 높은 상위 5명만 조회
-- * ROWNUM : 오라클에서 제공해주는 컬럼, 조회된 순서대로 1부터 순번을 부여해주는 컬럼

SELECT  ROWNUM, EMP_NAME, SALARY -- 순번이 부여되어 정렬도 하기전에 이미 됨.
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;
-- 정삭적인 결과가 조회 X

-- OREDER BY절이 수행된 결과를 가지고 ROWUM 부여 후 5명 추림
...
-- ROWNUM이랑 전체컬럼 조회하고 싶은데 X => 별칭부여하는 방식으로 해결
SELECT ROWNUM, E.*
FROM (SELECT EMP_NAME, SALARY, DEPT_CODE
        FROM EMPLOYEE
...
--SELECT *
--FROM (SELECT EMP_CODE, AVG(SALARY)
--        FROM EMPLYEE
--        ORDER BY AVG(SALARY));
...
SELECT /*ROWNUM 생략가능*/, DEPT_CODE, 평균급여 -- AVG(SALARY) 진짜 평균을 구하기때문에 별칭으로 해야 오류가 없음
...
/*
    ...
    - RANK() OVER(정렬기준) : 동일한 순위 이 후 등수를 동일한 인원수 만큼 건너 뀌고 순위 계산
...
FROM EMPLOYEE;
-- 공동 19위가 2명 그 뒤의 순위는 20 => 마지막 순위랑 조회된 행수가 다름
